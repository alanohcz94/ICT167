 ...
 *   Date 12/5/2021
 *
 *   Description: Finishing and completing writeToCSV function and ensure all code is runnable
 *   Changes: Updated the BodyOfProgram() completed the writeToCsv in BodyOfProgram()
 *   Version:   3
 *   Date 13/5/2021
 *
 */
...
public class Menu {
    ...
    //search for the customer name if it exist in the list
    ...
    //the amount of coins provided and being split up into denotations
    ...
    private void writeToCSV(int choice){
        try {
            FileWriter fw = new FileWriter("coins.csv");
            BufferedWriter bw = new BufferedWriter(fw);
            PrintWriter pw = new PrintWriter(bw);

            for(Customer c : customerList) {
                String result = denoCal(c.getCoins(),choice);
                pw.println(c.getName()+","+result);
            }
            pw.flush();
            pw.close();
        } catch (Exception e){
            System.out.println("Error in writing into CSV file.");
        }
    }
